package beginner_2_2Review.NM400.nm004.하나둘셋더하기;
...
public class Solved2 {
	public static void main(String[] args) throws NumberFormatException, IOException {
		...
		for(int x : input) {
			System.out.printf("==========f(%d)검증 시작==========\n", x);
			System.out.println(re(x, dp));
			System.out.printf("==========f(%d)검증 완료==========\n", x);
		}
	...
	static int re(int num, int[] dp) {
		...
		if(dp[num]!=0) {
			System.out.printf("%s값 존재 = %d \n", "f("+num+")", dp[num]);
			return dp[num];
		} 
		System.out.printf("%s값 미존재 = 다음 하위 트리 이동 \n", "f("+num+")");
